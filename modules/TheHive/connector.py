...
class TheHiveConnector:
    ...
    def test_connection(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.error(
                    f"Failed to connect to TheHive. Status code: {response.status_code}"
                )
    ...
    def craftCommTask(self):
        ...
        commTask = CaseTask(title="Communication", status="InProgress", owner="synapse")
    ...
    def craftAlertArtifact(self, **attributes):
        ...
        if data_type == "fqdn":
            if not is_valid_fqdn(data_value):
                self.logger.debug(
                    f"Downgrading invalid FQDN '{data_value}' to 'hostname'"
                )
                data_type = "hostname"
    ...
    def addFileObservable(self, esCaseId, filepath, comment):
        ...
        response = self.theHiveApi.create_case_observable(esCaseId, file_observable)
    ...
    def createAlert(self, alert):
        ...
        if response.status_code == 201:
            self.logger.info(
                f"Alert successfully created in TheHive. ID: {response.json()['id']}"
            )
            return response.json()
        else:
    ...
    def addObservable(self, esCaseId, datatype, ioc_list, tags, comment):
        ...
        response = self.theHiveApi.create_case_observable(esCaseId, ioc_observable)
    ...
    def splitDescription(self, description):
        ...
        for section in split_description[1:]:
            ...
            if title in output_dict:
                self.logger.warning(
                    "Section was already found in the output_dict, it will be overwritten."
                )
    ...
    def compareDescriptions(self, old_description, new_description):
        ...
        for section in split_new_alert_description:
            ...
            if section in split_current_alert_description:
                if (
                    split_current_alert_description[section]
                    != split_new_alert_description[section]
                ):
                    ...
                    self.logger.debug(
                        "New description is identical to old description."
                    )
            else:
    ...
    def checkIfInCase(self, source_ref):
        ...
        self.logger.debug(
            "Checking if third party ticket({}) is linked to a case".format(source_ref)
        )
    ...
    def checkIfUpdated(self, current_a, new_a):
        ...
        for item in sorted(new_a):
            ...
            if item == "artifacts":
                ...
                current_a[item] = [
                    i for i in current_a[item] if not ("uc_enrichment" in i["tags"])
                ]
                # If the array is of different size an update is required
                if not len(current_a[item]) == len(new_a[item]):
                    self.logger.info(
                        "Length mismatch detected: old length:%s, new length: %s"
                        % (len(current_a[item]), len(new_a[item]))
                    )
                ...
                for i in range(len(new_a[item])):
                    ...
                    for attribute in self.vars_new_artifacts:
                        if (
                            self.vars_current_artifacts[attribute]
                            != self.vars_new_artifacts[attribute]
                        ):
                            self.logger.debug(
                                "Change detected for %s, new value: %s"
                                % (
                                    self.vars_current_artifacts[attribute],
                                    self.vars_new_artifacts[attribute],
                                )
                            ...
                            self.logger.debug(
                                "old: %s, new: %s"
                                % (self.vars_current_artifacts, self.vars_new_artifacts)
                            )
                            list_of_updated_items.append("artifacts")
            ...
            if item == "tags":
                ...
                self.diff = list(
                    itertools.filterfalse(
                        lambda x: x in new_a["tags"], current_a["tags"]
                    )
                )
                self.diff = self.diff + list(
                    itertools.filterfalse(
                        lambda x: x in current_a["tags"], new_a["tags"]
                    )
                )
                if len(self.diff) > 0:
            ...
            if item == "description":
                ...
                try:
                    current_alert_description, current_alert_enrichment_table = (
                        retrieveSplittedDescription(current_a["description"])
                    )
                ...
                if current_alert_description != new_alert_description:
                    ...
                    updated_alert_description["new_descriptions"] = (
                        self.compareDescriptions(
                            current_alert_description, new_alert_description
                        )
                    )

                    # Prepare updated description
                    if enrichment_table_found:
                        updated_alert_description["description"] = (
                            new_alert_description
                            + "#### Enriched data"
                            + current_alert_enrichment_table
                        )
                    else:
    ...
    def checkForUpdates(self, alert_generated, alert_found, source_ref):
        ...
        updated_fields, updated_description = self.checkIfUpdated(
            alert_found, vars(alert_generated)
        )
        if len(updated_fields) > 0:
            self.logger.info(
                f"Found changes for {alert_found['id']} in these fields: {updated_fields}"
            )
            ...
            if (
                found_case
                and "resolutionStatus" in found_case
                and found_case["resolutionStatus"] == "Duplicated"
            ):
                self.logger.info(
                    f"Closing original case({found_case['id']}) as The Hive has reopened it"
                )
                self.closeCase(found_case["id"])

            if updated_description and found_case:
                self.logger.info(
                    f'Alert {alert_found["id"]} is part of an existing case: {found_case["id"]}'
                )
                # if case is present. Find and replace the new description field content there as well
                if (
                    "resolutionStatus" in found_case
                    and found_case["resolutionStatus"] == "Duplicated"
                ):
                    self.logger.info(
                        f'Update mechanism found merged case {found_case["id"]}, looking for top merged case'
                    )
                    merged_case_found = self.getFinalMergedCase(found_case)
                    self.logger.info(
                        f'Found the top merged case {merged_case_found["id"]}, adding update there'
                    )
                    if merged_case_found:
                        found_case = merged_case_found
                self.addUpdateToCase(
                    found_case, source_ref, updated_description["new_descriptions"]
                )
            else:
    ...
    def addUpdateToCase(self, case, incident_number, updated_description):
        ...
        if update_task_dict is None:
            self.logger.debug(
                f'Update task not found in case {case["id"]}, creating "Update" task'
            )
            update_task = CaseTask(
                title=task_title,
                status="InProgress",
                flag=True,
                owner="synapse",
    ...
    def runAnalyzer(self, cortex_id, artifact_id, analyzer_id):
        ...
        if response.status_code == 201:
            ...
            self.handleErrors(
                f"Failed to run analyzer {analyzer_id} on artifact {artifact_id}",
                response,
            )
