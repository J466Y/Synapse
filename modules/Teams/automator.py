    ...
    def SendNotificationFromAlert(self, action_config, webhook):
        ...
        if self.cfg.getboolean("Automation", "enable_customer_list", fallback=False):
            ...
            try:
                ...
                self.logger.warning(
                    "Could not retrieve url for customer %s, using default"
                    % self.customer_id
                )
        else:
        ...
        if hasattr(self, "url"):
            self.teams_data = {}
            self.teams_data["text"] = "***%s***</br><pre>%s</pre>" % (
                self.title,
            ...
            try:
                self.response = requests.post(
                    self.url,
                    data=json.dumps(self.teams_data),
                    headers={"Content-Type": "application/json"},
                    timeout=60,
                )
                if self.response.status_code not in [200, 201]:
                    self.logger.error(
                        "Something went wrong when posting to teams: %s"
