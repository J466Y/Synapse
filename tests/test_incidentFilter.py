...
class EntityTemplate:
    def __init__(self, kind, properties):
        entity_template = {"kind": kind, "properties": properties}
...
def test_titleFilter_contains_all(incidentFilter):
    ...
    assert (
        func([{"contains_all": ["A", "B"]}, {"contains_all": ["C", "D"]}], "CDEF")
        is True
    )
    assert (
        func([{"contains_all": ["A", "B"]}, {"contains_all": ["C", "D"]}], "ADEF")
        is False
    )
...
def test_checkEntityFilter_input(incidentFilter):
    ...
    entitity_inclusions = [
        {"entity": {"kind": "Host", "properties": {"hostName": "test123"}}}
    ]
    with pytest.raises(TypeError):
        func({"test": "value"}, EntityTemplate("Host", {"hostName": "test123"}).entity)
        func(
            [{"test": "value"}], EntityTemplate("Host", {"hostName": "test123"}).entity
        )
...
def test_checkEntityFilter(incidentFilter):
    ...
    entitity_inclusions = [
        {"entity": {"kind": "Host", "properties": {"hostName": "test123"}}},
        {"entity": {"kind": "File", "properties": {"fileName": "test123.txt"}}},
        {
            "entity": {
                "kind": "File",
                "properties": {"fileName": ["test456.txt", "test321.txt"]},
            }
        },
        {
            "entity": {
                "kind": "Account",
                "properties": {"accountName": [{"regex": "test.*"}]},
            }
        },
    ]
    ...
    assert (
        func(
            entitity_inclusions, EntityTemplate("Host", {"hostName": "test123"}).entity
        )
        is True
    )
    assert (
        func(
            entitity_inclusions, EntityTemplate("File", {"hostName": "test123"}).entity
        )
        is False
    )
    assert (
    ...
    assert (
        func(
            entitity_inclusions,
            EntityTemplate("File", {"fileName": "test456.txt"}).entity,
        )
        is True
    )
    assert (
    ...
    assert (
        func(
            entitity_inclusions,
            EntityTemplate("Account", {"accountName": "test123456.txt"}).entity,
        )
        is True
    )
    ...
    assert (
        func(
            entitity_inclusions,
            [
                EntityTemplate("Host", {"hostName": "test123"}).entity,
                EntityTemplate("File", {"fileName": "test123.txt"}).entity,
                EntityTemplate("File", {"fileName": "test321.txt"}).entity,
                EntityTemplate("Account", {"accountName": "test123456.txt"}).entity,
            ],
...
def test_checkCustomDetails(incidentFilter):
    ...
    assert (
        func(
            {"SOCalert": "True"},
            [{"SOCalert": ["True", "False", "False", "False", "False"]}],
        )
        is True
    )
    assert (
        func(
            {"SOCalert": "True"},
            [{"SOCalert": ["False", "False", "False", "False", "False"]}],
        )
        is False
    )
...
def test_checkInclusion(incidentFilter):
    ...
    incidentFilter.inclusion_config = [
        {
            "Inclusion Name": {
                "product": "Defender",
                "severity": "High",
    ...
    assert (
        func(
            AzureIncident(
                ["Defender"], "High", "Suspicious UserAccount Detected"
            ).incident,
            "",
        )[0]
        is True
    ...
    assert (
        func(
            AzureIncident(
                ["Defender"], "High", "Suspicious AdminAccount Detected"
            ).incident,
            "",
        )[0]
        is False
    ...
    assert (
        func(
            AzureIncident(
                ["Defender"], "Medium", "Suspicious UserAccount Detected"
            ).incident,
            "",
        )[0]
        is False
    ...
    assert (
        func(
            AzureIncident(
                ["Defender Extended"], "High", "Suspicious UserAccount Detected"
            ).incident,
            "",
        )[0]
        is False
    ...
    assert (
        func(
            AzureIncident(
                ["Sentinel"], "Undetermined", "Suspicious AdminAccount Detected"
            ).incident,
            "",
        )[0]
        is True
    ...
    assert (
        func(
            AzureIncident(
                ["Defender"], "Unique", "Suspicious AdminAccount Detected"
            ).incident,
            "",
        )[0]
        is True
    ...
    assert (
        func(
            AzureIncident(
                ["Defender"], "Undetermined", "Unique Account Detected"
            ).incident,
            "",
        )[0]
        is True
...
def test_checkExclusion(incidentFilter):
    func = incidentFilter.checkFilter
    incidentFilter.exclusion_config = [
        {
            "Exclusion Name": {
                "product": "Defender",
                "severity": "High",
...
def test_filterIncident(incidentFilter):
    ...
    incidentFilter.exclusion_config = [
        {"severity exclusion": {"severity": "Informational"}},
        {"title exclusion": {"title": "SEC"}},
    ]

    assert (
        func(
            AzureIncident(["Defender"], "High", "Unique UserAccount Detected").incident
        )[0]
        is True
    )
    assert (
        func(
            AzureIncident(
                ["Defender"], "Medium", "Unique UserAccount Detected"
            ).incident
        )[0]
        is True
    )
    assert (
        func(
            AzureIncident(["Defender"], "Low", "Unique UserAccount Detected").incident
        )[0]
        is True
    )
    assert (
        func(
            AzureIncident(
                ["Sentinel"], "Informational", "Unique UserAccount Detected"
            ).incident
        )[0]
        is False
    )
    assert (
        func(AzureIncident(["MCAS"], "Unique", "Unique UserAccount Detected").incident)[
            0
        ]
        is True
    )
    assert (
        func(
            AzureIncident(
                ["Sentinel"], "Unique", "SEC1234: Unique UserAccount Detected"
            ).incident
        )[0]
        is False
    )
    assert (
        func(AzureIncident(["Defender for 365"], "Unknown", "Unknown").incident)[0]
        is False
    )
