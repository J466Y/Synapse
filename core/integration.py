...
class Main:
        ...
        for artifact in artifacts:
            if tlp_modifiers:
                ...
                for tlp, tlp_config in tlp_modifiers.items():
                    ...
                    self.tlp_table = {"white": 0, "green": 1, "amber": 2, "red": 3}
                        ...
                        if observable_type == "ip" and artifact["dataType"] == "ip":
                            for entry in observable_type_config:
                                ...
                                try:
                                    ...
                                    if entry[-3:] == "/32":
                                        self.tlp_list_entry = ipaddress.ip_address(
                                            entry[:-3]
                                        )
                                        self.match = (
                                            observable_ip == self.tlp_list_entry
                                        )
                                    ...
                                    # Capture actual network entries
                                        ...
                                        self.tlp_list_entry = ipaddress.ip_network(
                                            entry, strict=False
                                        )
                                        self.match = (
                                            observable_ip in self.tlp_list_entry
                                        )

                                    # If matched add it to new entries to use outside of the loop
                                    if self.match:
                                        self.logger.debug(
                                            "Observable {} has matched {} through {} of the TLP modifiers list. Adjusting TLP...".format(
                                                artifact["data"], tlp, entry
                                            )
                                        )
                                        artifact["tlp"] = self.tlp_int
                                except Exception as e:
                                    self.logger.warning(
                                        f"Failed to process IP {artifact['data']} for TLP check: {e}"
                                    )
                            ...
                            for extraction_regex in observable_type_config:
                                ...
                                if self.regex.search(artifact["data"]):
                                    self.logger.debug(
                                        "Observable {} with type {} has matched {} through {} of the TLP modifiers list. Adjusting TLP...".format(
                                            artifact["data"],
                                            observable_type,
                                            tlp,
                                            extraction_regex,
            ...
            if "tlp" not in artifact:
                artifact["tlp"] = self.cfg.get(
                    "Automation", "default_observable_tlp", fallback=2
                )
    ...
    def checkObservableExclusionList(self, artifacts):
        self.artifacts = []
        self.exclusions = self.cfg.get(
            "Automation", "observable_exclusions", fallback=None
        )
        if self.exclusions:
            ...
            for artifact in artifacts:
                ...
                if self.match_found:
                    self.logger.debug(
                        "Observable {} with type {} has matched through {} of the exclusion list. Ignoring observable...".format(
                            artifact["data"], artifact["dataType"], self.matched_on
                        )
                    )
                    continue
                # Add artifact to an array again
    ...
    def checkIfInClosedCaseOrAlertMarkedAsRead(self, sourceref):
        ...
        self.logger.debug(
            "Checking if third party ticket({}) is linked to a closed case".format(
                sourceref
            )
        )
        ...
        if len(alert_results) > 0:
            ...
            if alert_found["status"] == "Ignored":
                ...
                return {
                    ...
                    "summary": "Closed by Synapse with summary: Marked as Read within The Hive",
                }
                ...
                if case_found and case_found.get("status") == "Resolved":
                    if case_found.get("resolutionStatus") != "Duplicated":
                        self.logger.info(
                            f"{sourceref} was found in a closed case {case_found.get('id')}"
                        )
                        ...
                        self.logger.info(
                            f"{sourceref} was found in a duplicated case {case_found['id']}"
                        )
                        ...
                        if merged_case_found["status"] == "Resolved":
                            if "resolutionStatus" in merged_case_found:
                                resolution_status = merged_case_found[
                                    "resolutionStatus"
                                ]
    ...
    def getFinalMergedCase(self, duplicated_case, handled_cases=[]):
        ...
        if "mergeInto" in duplicated_case:
            ...
            if "resolutionStatus" in case_found:
                if (
                    case_found["resolutionStatus"] == "Duplicated"
                    and merged_into not in handled_cases
                ):
