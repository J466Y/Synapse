...
class Automation:
        ...
        if len(alert_results) > 0:
            ...
            if alert_found["status"] == "Ignored":
                ...
                if case_found["status"] == "Resolved":
                    if (
                        "resolutionStatus" in case_found
                        and case_found["resolutionStatus"] == "Duplicated"
                    ):
    ...
    def parse_hooks(self):
        ...
        if (
            self.webhook.isClosedAzureSentinelCase()
            ...
            if self.webhook.data["operation"] == "Delete":
                ...
                self.classification_comment = (
                    "Closed by Synapse with summary: Deleted within The Hive"
                )
                ...
                logger.info(
                    "Alert {} has been marked as read".format(
                        self.webhook.data["object"]["sourceRef"]
                    )
                )
                self.AzureSentinelConnector.closeIncident(
                    self.incidentId, self.classification, self.classification_comment
                )

            # Ensure duplicated incidents don't get closed when merged, but only when merged case is closed
            elif (
                "resolutionStatus" in self.webhook.data["details"]
                and self.webhook.data["details"]["resolutionStatus"] != "Duplicated"
            ):
