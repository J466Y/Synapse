...
class AzureSentinelConnector:
    ...
    def __init__(self, cfg):
        ...
        self.base_url = "https://management.azure.com/subscriptions/{}/resourceGroups/{}/providers/Microsoft.OperationalInsights/workspaces/{}/providers/Microsoft.SecurityInsights".format(
    ...
    def getBearerToken(self):
        self.url = "https://login.microsoftonline.com/{}/oauth2/token".format(
            self.tenant_id
        )
        self.data = "grant_type=client_credentials&client_id={}&client_secret={}&resource=https%3A%2F%2Fmanagement.azure.com&undefined=".format(
        ...
        try:
            self.response = requests.post(
                self.url, self.data, headers=self.headers, timeout=60
            )
            ...
            self.logger.error(
                "Could not get Bearer token from Azure Sentinel: {}".format(e)
            )
    ...
    def azureRequest(self, method, url, data=None, bearer_token_regenerated=False):
        ...
        if not url.startswith("https://management.azure.com"):
            self.logger.error(
                "Blocked potentially malicious SSRF attempt to URL: {}".format(url)
            )
            return {
                "success": False,
                "status_code": 403,
                "json": {"error": "SSRF Blocked"},
            }
        ...
        # Prepare return dict
        result = {"success": False, "status_code": 0, "json": {}}
        ...
        try:
            ...
            try:
                ...
                self.logger.debug(
                    "no json found in request body or could not parse the json"
                )
            if "error" in response.json() and response.status_code != 401:
                self.logger.error(
                    "Azure Sentinel prompted an error code '{}' with full error: '{}'".format(
            ...
            if response.status_code == 200:
                result["success"] = True
                self.logger.debug(
                    "the following response was received: {}".format(result["json"])
                )
                return result
            ...
            self.logger.error(
                "AzureSentinel returned http {} with the following body: {}".format(
            ...
            if not bearer_token_regenerated:
                ...
                self.bearer_token = self.getBearerToken()
                return self.azureRequest(
                    method, url, data, bearer_token_regenerated=True
                )
            else:
                self.logger.error(
                    "An attempt was made to regenerate the Bearer token, but the request still fails with code 401: {}".format(
                        e
                    )
                )
                return result
            ...
            self.logger.error(
                "Could not retrieve data from Azure Sentinel: {}".format(e),
                exc_info=True,
            )
            return result
    ...
    def formatDate(self, target, sentinelTimeStamp):
        ...
        self.logger.debug(
            "Found timezone to convert to: {}".format(configured_timezone)
        )
        ...
        self.logger.debug(
            "Cropped timestamp from: {} to: {}".format(
                sentinelTimeStamp, sentinelTimeStamp[0:19]
            )
        )
        ...
        self.logger.debug(
            "Changed time from {} to {}".format(
                sentinelTimeStamp, string_formatted_time
            )
        )
    ...
    def getIncidents(self):
        ...
        url = (
            self.base_url
            + "/incidents?api-version=2020-01-01&%24filter=(properties%2Fstatus%20eq%20'New'%20or%20properties%2Fstatus%20eq%20'Active')&%24orderby=properties%2FcreatedTimeUtc%20desc"
        )
        ...
        if response["success"]:
            ...
            if self.cfg.getboolean("AzureSentinel", "filter_incidents"):
                ...
                for incident in enriched_incidents:
                    ...
                    passed_filter, filter_message = self.incidentFilter.filterIncident(
                        incident
                    )
                    if passed_filter is False:
                        if self.cfg.getboolean(
                            "AzureSentinel", "close_filtered_incident"
                        ):
                            alert_products_to_auto_close = self.cfg.get(
                                "AzureSentinel", "close_only_for_alert_products"
                            )
                            if alert_products_to_auto_close:
                                can_close = []
                            # Check to see if the alert products are allowed to be closed
                            if not self.incidentFilter.checkProductFilter(
                                alert_products_to_auto_close,
                                incident["properties"]["additionalData"][
                                    "alertProductNames"
                                ],
                            ):
                                continue
                                ...
                                self.logger.info(
                                    f"Closing incident {incident['name']} due to a match in the filter"
                                )
                                self.closeIncident(
                                    incident["name"],
                                    "Undetermined",
                                    f"Closed by Synapse due to incident filtering.\nReason:\n{filter_message}",
                                )
                        else:
                            self.logger.info(
                                f"Ignoring incident {incident['name']} due to a match in the filter"
                            )
                        continue
                    else:
                        self.logger.info(
                            f"Incident {incident['name']} not filtered, adding incident to list"
                        )
    ...
    def enrichAlert(self, alert):
        ...
        if (
            self.incidentFilter.checkProductFilter(
                "Azure Sentinel", [alert["properties"]["productName"]]
            )
            is True
        ):
            ...
            if enriched_alert is not False:
                if "query" in enriched_alert:
                    ...
                    if first_event is not False:
                        ...
                        self.logger.debug(
                            f'Could not find raw events for {alert["name"]}'
                        )
                else:
    ...
    def updateIncidentStatusToActive(self, incident_id):
        ...
        if response["success"]:
            ...
            if incident["properties"]["status"] == "Active":
                self.logger.info(
                    "Incident {} is already marked as Active".format(incident_id)
                )
                return True
            else:
                data = {
                ...
                if response["success"]:
                    self.logger.debug(
                        "Incident {} is now marked as Active".format(incident_id)
                    )
                    return True
                else:
                    self.logger.error(
                        "Failed to update incident %s", incident_id, exc_info=True
                    )
                    return False
        else:
            self.logger.error(
    ...
    def getRule(self, uri):
        ...
        if response["success"]:
            ...
            self.logger.error(
                "Could not retrieve rule information from Azure Sentinel", exc_info=True
            )
            return False
    ...
    def getRelatedAlerts(self, incident_id):
        ...
        if response["success"]:
            ...
            if "value" in result and len(result["value"]) > 0:
                ...
                self.logger.debug(
                    "Azure Sentinel provided no results for related alerts"
                )
                return False
        else:
            self.logger.warning(
                "Could not retrieve/find related alerts from Azure Sentinel"
            )
            return False
    ...
    def getEntities(self, incident_id):
        ...
        url = (
            ...
            + "/incidents/"
            + incident_id
            + "/entities?api-version=2021-04-01"
        )
        ...
        if response["success"]:
            ...
            self.logger.warning(
                "Could not retrieve/find entity information from Azure Sentinel"
            )
            return False
    ...
    def queryLogAnalytics(self, data):
        ...
        url = "https://management.azure.com/subscriptions/{}/resourceGroups/{}/providers/Microsoft.OperationalInsights/workspaces/{}/query".format(
            self.subscription_id, self.resource_group, self.workspace
        ) + "?api-version={}".format(
            "2017-10-01"
        )
        self.logger.debug(
            "Querying Log Analytics {} with data:\n{}".format(
        ...
        if response["success"]:
            ...
            if "tables" in result and len(result["tables"]) > 0:
                self.logger.debug(
                    "Received the following raw results from Azure Sentinel: {}".format(
    ...
    def beautifyQueryResults(self, results):
            ...
            for row in table["rows"]:
                ...
                for vc, row_value in enumerate(row):
                    if isinstance(row_value, str) and (
                        row_value.startswith("[")
                        and row_value.endswith("]")
                        or row_value.startswith("{")
                        and row_value.endswith("}")
                    ):
                        row_value = row_value.replace("\\r\\n", "\n")
                        try:
                            results[table["columns"][vc]["name"]] = json.loads(
                                row_value
                            )
                        except json.decoder.JSONDecodeError:
                            self.logger.warning(
                                f"Could not parse json for: {row_value}"
                            )
        ...
        self.logger.debug(
            "Parsed the following beautified results from the query: {}".format(
