    ...
    def parse_hooks(self):
        ...
        if "synapse:close-offense" in tags:
            ...
            if obj_type == "alert":
                ...
                if self.offense_id:
                    logger.info(
                        f"Triggering explicit QRadar offense closure for alert offense {self.offense_id}"
                    )
                    self.QRadarConnector.closeOffense(self.offense_id)

            elif obj_type == "case":
                case_id = self.webhook.data["object"].get(
                    "id", self.webhook.data.get("objectId")
                )
                ...
                if self.webhook.fromQRadar(case_id):
                    # Single alert case
                    if (
                        hasattr(self.webhook, "ext_alert_id")
                        and self.webhook.ext_alert_id
                    ):
                        logger.info(
                            f"Triggering explicit QRadar offense closure for case {case_id} -> offense {self.webhook.ext_alert_id}"
                        )
                        self.QRadarConnector.closeOffense(self.webhook.ext_alert_id)

                    # Multiple alerts case
                    elif (
                        hasattr(self.webhook, "ext_alert_ids")
                        and len(self.webhook.ext_alert_ids) > 0
                    ):
                        logger.info(
                            f"Triggering explicit QRadar bulk offense closure for case {case_id} -> offenses {self.webhook.ext_alert_ids}"
                        )
                        for offense_id in self.webhook.ext_alert_ids:
                            logger.info(
                                f"Closing offense {offense_id} for case {case_id}"
                            )
        ...
        if (
            self.webhook.isClosedQRadarCase()
            or self.webhook.isDeletedQRadarCase()
            or self.webhook.isQRadarAlertMarkedAsRead()
        ):
            operation = self.webhook.data.get("operation") or self.webhook.data.get(
                "action"
            )
            ...
            if hasattr(self, "case_id"):
                if hasattr(self.webhook, "ext_alert_id"):
                    logger.info(
                        "Closing offense {} for case {}".format(
