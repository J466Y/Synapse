    ...
    def nslookup(self, action_config, webhook):
        ...
        for query_name, query_config in self.query_config.items():
            try:
                ...
                self.logger.info(
                    "variables in the query: {}".format(query_config["query"])
                )
                # Render query
                try:
                    ...
                    self.logger.info(
                        "Replaced variables: {}".format(
                            self.query_variables["input"]["Offense_Source"]
                        )
                    )
                    try:
                        self.enrichment_result = socket.gethostbyaddr(
                            str(self.query_variables["input"]["Offense_Source"])
                        )
                    except Exception as e:
                        self.logger.warning(
                            "Could not run socket.gethostbyaddr", exc_info=True
                        )
