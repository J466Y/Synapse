...
automation_config = loadAutomationConfiguration(
    cfg.get("Automation", "automation_config_dir", fallback=None)
)
automation_list = []
...
def verify_api_key():
    ...
    if api_key:
        ...
        if not auth_header or not auth_header.startswith("Bearer "):
            logger.warning(
                "Missing or invalid Authorization header from {}".format(
                    request.remote_addr
                )
            )
            return (
        ...
        try:
            ...
            logger.warning(
                "Malformed Authorization header from {}".format(request.remote_addr)
            )
            return (
                jsonify(
...
def endpoint(integration):
    try:
        ...
        integration = modules["integration"][
            enabled_integration_modules[integration]
        ].Integration()
        response = integration.validateRequest(request)
        return response
    except KeyError:
        logger.warning(
            "Integration module not found or disabled: {}".format(integration)
        )
        return "ERROR: Integration module not found or disabled", 404
    except Exception as e:
        logger.error(
            "Something went wrong in integration {}: {}".format(integration, e),
            exc_info=True,
        )
        ...
        try:
            ...
            if queue_info_request["type"] in ["all", "memory"]:
                ...
                for queue_item in queue_content:
                    ...
                    queue_info["memory_queue"].append(
                        QueueLog(
                            queue_item.time,
                            queue_item.priority,
                            queue_item.action,
                            queue_item.argument[0],
                        )._asdict()
                    )
            if queue_info_request["type"] in ["all", "persistent"]:
                ...
                for persistent_queue_item in persistent_queue_content:
                    queue_info["persistent_queue"].append(
                        QueueLog(
                            persistent_queue_item.time,
                            persistent_queue_item.priority,
                            persistent_queue_item.action,
                            persistent_queue_item.argument[0],
                        )._asdict()
                    )
        except Exception as e:
            logger.error("Failed to listen or action webhook: %s" % e, exc_info=True)
...
if __name__ == "__main__":
    app.run(
        debug=cfg.getboolean("api", "debug_mode"),
