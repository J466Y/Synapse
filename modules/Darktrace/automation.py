    ...
    def parse_hooks(self):
        ...
        if pbid and "synapse:acknowledge-breach" in tags:
            ...
            if ack_result.get("status"):
                action_performed = {
                    "status": True,
                    "message": f"Acknowledge: Success for breach {pbid}",
                }

                # If there is a comment, add it
                if comment:
                    self.logger.info(
                        f"Adding comment to Darktrace breach {pbid}: {comment}"
                    )
                    ...
                    if comment_result.get("status"):
                        ...
                        action_performed[
                            "message"
                        ] += f" | Failed to add comment: {comment_result.get('data')}"
            else:
                action_performed = {
                    "status": False,
                    "message": f"Acknowledge: Failed for breach {pbid}. Error: {ack_result.get('data')}",
                }
