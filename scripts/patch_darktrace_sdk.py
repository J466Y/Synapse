...
LITERAL_REPLACEMENTS = [
    ...
    ("_Unset | None | float | tuple[float, float]",             "Union[_Unset, None, float, tuple[float, float]]"),
    ...
    ("tuple[dict[str, str], dict[str, Any] | None]",            "tuple[dict[str, str], Optional[dict[str, Any]]]"),
...
def fix_typing_and_future(content: str) -> str:
    ...
    for line in lines:
        if line.startswith('from __future__ import'):
            ...
            imported = set(i.strip() for i in line.replace('from typing import', '').split(','))
            if needs_union: imported.add('Union')
            if needs_optional: imported.add('Optional')
    ...
    if not typing_import_found and (needs_union or needs_optional):
        to_import = sorted((['Union'] if needs_union else []) + (['Optional'] if needs_optional else []))
...
def patch_file(filepath: str) -> bool:
    ...
    def safe_union_replacer(match):
        ...
        if any(c in expr for c in ['(', ')', '"', "'"]): return expr
        parts = [p.strip() for p in expr.split('|')]
        if any(re.match(r'^\d+$', p) for p in parts): return expr
    ...
    def patch_line(line):
        ...
        if re.search(r':\s*[\w\[\]|. ]+|->|^\s*\w+\s*=\s*[\w\[\]|. ]+', line):
            return re.sub(r'(?:[\w\.\[\]]+\s*\|\s*)+[\w\.\[\]]+', safe_union_replacer, line)
        return line
