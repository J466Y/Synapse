...
class RestApiClient:
    ...
    def call_api(
        self, endpoint, method, headers=None, params=[], data=None, print_request=False
    ):
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                self.logger.debug(
                    "the following response was received: {}".format(response.json())
                )
                return response
            ...
            self.logger.error(
                "QRadar returned http {} with the following body: {}".format(
            ...
            self.logger.error(
                "Could not retrieve data from QRadar: {}".format(e), exc_info=True
            )
            raise QRadarError
