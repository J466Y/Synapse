...
from thehive4py.models import Case, CustomFieldHelper, CaseTask
    ...
    def parse_hooks(self):
        ...
        if self.webhook.isNewMispAlert():
            ...
            if supported_iocs:
                ...
                logger.info(
                    "Alert {} contains IOCs that are supported".format(alert_id)
                )
        ...
        if self.webhook.isNewMispArtifact():
            ...
            if observable["dataType"] in self.qr_config["supported_datatypes"]:
                ...
                logger.info(
                    "Launching analyzers for observable: {}".format(observable["_id"])
                )
                response = self.CortexConnector.runAnalyzer(
                    "Cortex-intern", supported_observable, "IBMQRadar_Search_Manual_0_1"
                )
                ...
                customFields = (
                    ...
                    .add_date(
                        "firstSearched",
                        case_data["customFields"]["firstSearched"]["date"],
                    )
                    .add_date("lastSearched", current_time)
                    .build()
                )
        ...
        if (
            self.webhook.isCaseArtifactJob()
            and self.webhook.isSuccess()
            ...
            logger.info(
                "Job {} is part of a case that has been tagged as MISP case and has just finished".format(
                    self.webhook.data["object"]["cortexJobId"]
                )
            )
            ...
            if (
                int(
                    float(
                        self.webhook.data["object"]["report"]["summary"]["taxonomies"][
                            0
                        ]["value"]
                    )
                )
                ...
                logger.info(
                    "Job {} contains hits, checking if a task is already present for this observable".format(
                        self.webhook.data["object"]["cortexJobId"]
                    )
                )
                ...
                # Observable + Link
                # Must change: Usar funciones de recoger el observable directamente de la API usando el conector de thehive
                observable = self.webhook.data["object"]["artifactId"]
                observable_link = (
                    #The Hive Host [string]
                    + "/index.html#!/case/"
                    + caseid
                    + "/observables/"
