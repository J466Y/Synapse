    ...
    def checkSiem(self, action_config, webhook):
        ...
        # Cases
            ...
            if self.supported_query_type in action_config:
                ...
                self.logger.debug(
                    "Search query config not found for the Imported Alert"
                )
                return False
        ...
        for query_name, query_config in self.query_config.items():
                    ...
                    for template_var in self.template_vars:
                        ...
                        if template_var == "Start_Time":
                            ...
                            if "start_time_offset" in query_config:
                                self.start_time = self.query_variables["input"][
                                    "Start_Time"
                                ]
                                self.query_variables["input"]["Start_Time"] = (
                                    self.parseTimeOffset(
                                        ...
                                        self.cfg.get("Splunk", "time_format"),
                                    )
                                )
                            else:
                                self.query_variables["input"]["Start_Time"] = (
                    ...
                    if self.results:
                        ...
                        for event in self.query_variables[query_name]["result"]:
                            ...
                            for field_key, field_value in event.items():
                                ...
                                self.table_data_row = (
                                    self.table_data_row + " %s &nbsp;|" % field_value
                                )
                ...
                if self.supported_query_type == "enrichment_queries":
                    ...
                    if self.results:
                        ...
                        self.enrichment_result = (
                            self.TheHiveAutomators.make_it_presentable(
                                self.query_variables[query_name]["result"][0][
                                    "enrichment_result"
                                ]
                            )
                        )
                    ...
                    try:
                        if (
                            ...
                            self.alert_description = self.TheHiveConnector.getAlert(
                                self.alert_id
                            )["description"]
            ...
            if self.enriched:
                ...
                self.TheHiveConnector.updateAlert(
                    self.alert_id, self.updated_alert, ["description"]
                )
