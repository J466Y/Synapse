...
class Automators(Main):
    ...
    def createMailTask(self, action_config, webhook):
        ...
        self.title = action_config["title"]
        self.description = self.renderTemplate(
            action_config["long_template"],
            self.tags,
            webhook,
        ...
        if (
            "auto_send_mail" in action_config
            and action_config["auto_send_mail"]
            and not self.stopsend
        ):
    ...
    def runAnalyzer(self, action_config, webhook):
        ...
        if webhook.isNewArtifact():
            ...
            if self.observable["dataType"] in action_config["datatypes"]:
                ...
                if (
                    self.observable["dataType"] == "ip"
                    and "blacklist" in action_config
                    and "ip" in action_config["blacklist"]
                ):
                    for entry in action_config["blacklist"]["ip"]:
                        ...
                        if match:
                            self.logger.debug(
                                "Observable {} has matched {} of blacklist. Ignoring...".format(
                                    self.observable["data"], entry
                                )
                            )
                            return True
                ...
                self.TheHiveConnector.runAnalyzer(
                    ...
                    self.supported_observable,
                    action_config["analyzer"],
                )
                return True
    ...
    def closeCaseForTaxonomyInAnalyzerResults(self, action_config, webhook):
        ...
        if webhook.isCaseArtifactJob() and webhook.isSuccess():
            ...
            self.logger.debug(
                "Job {} has just finished".format(webhook.data["object"]["cortexJobId"])
            )

            # Check if the result count higher than 0
            if (
                webhook.data["object"]["report"]["summary"]["taxonomies"][0]["level"]
                in action_config["taxonomy_level"]
            ):
                self.logger.info(
                    "Job {} has configured taxonomy level, checking if a task is already present for this observable".format(
                ...
                if self.case_data["status"] != "Resolved":
                    self.logger.info(
                        "Case is not yet closed, closing case for {} now...".format(
                            webhook.data["object"]["cortexJobId"]
                        )
                    )
                    # Close the case
    ...
    def createTaskForTaxonomyinAnalyzerResults(self, action_config, webhook):
        ...
        if webhook.isCaseArtifactJob() and webhook.isSuccess():
            ...
            if (
                ...
                self.observable_link = (
                    self.cfg.get("Automation", "hive_url", fallback="https://localhost")
                    + "/index.html# !/case/"
                    + self.caseid
                    + "/observables/"
                    + webhook.data["object"]["artifactId"]
                )

                # Task name
                self.casetask.title = "{} {}".format(
                    action_config["title"], self.observable
                )
                ...
                if not self.case_task_found:
                    self.logger.info(
                        "No task found, creating task for observable found in job {}".format(
                            webhook.data["object"]["cortexJobId"]
                        )
                    )
                    # Add description
                    self.casetask.description = action_config["description"]
                    self.casetask.description = (
                        self.casetask.description
                        + "\n\n {} is seen on {}\n".format(
    ...
    def autoCreateCase(self, action_config, webhook):
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to automatically promote alert: {alert_id}. Case could not be created."
            )
            return False
        return True
    ...
    def fieldExtraction(self, action_config, webhook):
        ...
        for field_name, field_config in action_config.items():
            ...
            if self.field_name == "task":
                continue
            self.logger.info(
                "Found the action for field_name: {}".format(self.field_name)
            )

            # let's search the value from the payload based on the regex
            try:
                self.logger.info(
                    "parsing value for field: {} with regex: {}".format(
                        self.field_name, self.field_config["regex"]
                    )
                )
                self.extracted_value = re.search(
                    self.field_config["regex"], self.payload.group(1)
                )

                if self.extracted_value is None:
                    self.fallback_regex = re.sub(
                        "'", "\\\\\\'", self.field_config["regex"]
                    )
                    self.logger.info(
                        "trying fallback: parsing value for field: {} with regex: {}".format(
                            self.field_name, self.fallback_regex
                        )
                    )
                    self.extracted_value = re.search(
                        self.fallback_regex, self.payload.group(1)
                    )

                if self.extracted_value is None:
                    self.logger.warning(
                        "No match found for field: {} in payload".format(
                            self.field_name
                        )
                    )
                    continue
                ...
                try:
                    ...
                    self.logger.warning(
                        "Regex for field: {} does not contain a capturing group. Using full match.".format(
                            self.field_name
                        )
                    )
                ...
                self.logger.debug(
                    "parsed value: {} for field: {} with regex: {}".format(
                        matched_v, self.field_name, self.field_config["regex"]
                    )
                )
                ...
                if url_list:
                    self.clean_enrichment_results = (
                        "(" + ", ".join(['"{​​​​​}​​​​​"'.format(x) for x in url_list]) + ")"
                    )
                else:
                    self.clean_enrichment_results = (
                        "("
                        + ", ".join(
                            ['"{​​​​​}​​​​​"'.format(x) for x in dedup_extracted_value_list]
                        )
                        + ")"
                    )

                self.logger.info(
                    f"value of self.clean_enrichment_results: {self.clean_enrichment_results}"
                )

                #  Add results to description
                try:
                    with _alert_update_lock:
                        self.regex_end_of_table = r" \|\\n\\n\\n"
                        self.end_of_table = " |\n\n\n"
                        ...
                        self.th_alert_description = self.TheHiveConnector.getAlert(
                            self.alert_id
                        )["description"]
                        self.alert_description = re.sub(
                            self.regex_end_of_table,
                            self.replacement_description,
                            self.th_alert_description,
                        )
                        self.enriched = True
                        ...
                        self.TheHiveConnector.updateAlert(
                            self.alert_id, updated_alert, ["description"]
                        )
                        self.logger.debug(
                    ...
                    self.logger.warning(
                        "Could not add results from the query to the description. Error: {}".format(
                            e
                        )
                    )
                    raise
                ...
                if self.field_config["create_artifact"]:
                    ...
                    self.rule_secid = [
                        x for x in webhook.data["object"]["tags"] if x.startswith("SEC")
                    ]
                    ...
                    for v in dedup_extracted_value_list:
                        new_artifacts.append(
                            {
                                "data": v,
                                "dataType": field_config["datatypes"],
                                "message": "offense enrichment",
                                "tags": self.tags_list,
                                "tlp": 2,
                            }
                        )
                    ...
                    self.TheHiveConnector.updateAlert(
                        self.alert_id, updated_alert1, fields=["artifacts"]
                    )

            except Exception as e:
                self.logger.error(
                    "Could not extract the fields from payload. Error: {}".format(e),
                    exc_info=True,
                )
                continue
