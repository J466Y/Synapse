...
from cortex4py.query import Eq
...
class AnalyzersController(AbstractController):
    ...
    def get_by_type(self, data_type) -> List[Analyzer]:
        return self._wrap(
            self._api.do_get("analyzer/type/{}".format(data_type)).json(), Analyzer
        )

    def definitions(self) -> List[AnalyzerDefinition]:
        return self._wrap(
            self._api.do_get("analyzerdefinition").json(), AnalyzerDefinition
        )
    ...
    def run_by_id(self, analyzer_id, observable, **kwargs) -> Job:
        ...
        if observable.get("dataType") == "file":
            ...
            data = {"_json": json.dumps(post)}

            return self._wrap(
                self._api.do_file_post(
                    "analyzer/{}/run".format(analyzer_id),
                    data,
                    files=file_def,
                    params=params,
                ).json(),
            ...
            return self._wrap(
                self._api.do_post(
                    "analyzer/{}/run".format(analyzer_id), post, params
                ).json(),
                Job,
            )
