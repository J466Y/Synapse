...
STATE_FILE = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "backoff_state.json"
)
INITIAL_DELAY = 30
...
def run_sync():
    ...
    for module in MODULES:
        ...
        if current_time < m_state.get("next_attempt", 0):
            ...
            print(
                f"Saltando {name}: Servidor caído. Próximo intento en {wait_remaining} segundos."
            )
            continue
        ...
        try:
            ...
            response = requests.post(
                url, json=data, headers=headers, timeout=305
            )  # 5 min + 5s buffer
            ...
            if response.status_code == 200:
                ...
                if res_data.get("success"):
                    ...
                    m_state["failures"] += 1
                    delay = min(
                        INITIAL_DELAY * (2 ** (m_state["failures"] - 1)), MAX_DELAY
                    )
                    m_state["next_attempt"] = current_time + delay
                    print(
                        f"Resultado {name}: Servidor remoto DOWN. Aplicando backoff de {delay}s (Fallo #{m_state['failures']})"
                    )
                else:
                    print(
                        f"Resultado {name}: Error lógico - {res_data.get('message', 'No message')}"
                    )
            else:
                ...
                m_state["next_attempt"] = current_time + delay
                print(
                    f"Resultado {name}: HTTP {response.status_code}. Aplicando backoff de {delay}s"
                )
