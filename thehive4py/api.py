...
class TheHiveApi:
    ...
    def __find_rows(self, find_url, **attributes):
        ...
        # Add body
        data = {"query": attributes.get("query", {})}
    ...
    def update_case(self, case, fields=[]):
        ...
        data = {
            ...
            if (len(fields) > 0 and k in fields)
            or (len(fields) == 0 and k in update_keys)
        }
        try:
            return requests.patch(
                req,
    ...
    def create_task_log(self, task_id, case_task_log):
        ...
        if case_task_log.file:
            f = {
                "attachment": (
                    os.path.basename(case_task_log.file),
                    open(case_task_log.file, "rb"),
                    magic.Magic(mime=True).from_file(case_task_log.file),
                )
            }
            try:
                return requests.post(
            ...
            try:
                return requests.post(
                    req,
                    headers={"Content-Type": "application/json"},
                    data=json.dumps({"message": case_task_log.message}),
                    proxies=self.proxies,
                    auth=self.auth,
                    verify=self.cert,
                    timeout=60,
    ...
    def get_case(self, case_id):
        ...
        try:
            return requests.get(
                req, proxies=self.proxies, auth=self.auth, verify=self.cert, timeout=60
            )
        except requests.exceptions.RequestException as e:
    ...
    def get_case_observable(self, artifact_id):
        ...
        try:
            ...
            raise CaseObservableException(
                "Case observable fetch error (v1): {}".format(e)
            )
    ...
    def get_case_observables(self, case_id, **attributes):
        ...
        try:
            ...
            raise CaseObservableException(
                "Case observables fetch error (v1): {}".format(e)
            )
    ...
    def get_case_template(self, name):
        ...
        data = {"query": And(Eq("name", name), Eq("status", "Ok"))}
        ...
        try:
            ...
            if response.status_code == 200 and len(json_response) > 0:
                ...
                raise CaseTemplateException(
                    "Case template fetch error: Unable to find case template {}".format(
                        name
                    )
                )
        except requests.exceptions.RequestException as e:
    ...
    def promote_alert_to_case(self, alert_id):
        ...
        try:
            ...
            raise AlertException(
                "Couldn't promote alert to case: {}".format(e)
            )
    ...
    def merge_alert_into_case(self, alert_id, case_id):
        ...
        try:
            ...
            raise AlertException(
                "Couldn't merge alert into case: {}".format(e)
            )
    ...
    def search_responder_by_name(self, responder_name):
        ...
        :return: str
        """
        data = {"query": {"_string": responder_name}}

        req = self.url + "/api/connector/cortex/responder/_search"
        try:
            responder_list = requests.post(
                req,
                headers={"Content-Type": "application/json"},
                json=data,
                proxies=self.proxies,
