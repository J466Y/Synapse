    ...
    regex = [
        {
            "types": ["ip"],
            "regex": re.compile(
                r"(?:^|\D)((?:25[0-5]|2[0-4]\d|[1]\d\d|[1-9]\d|[0-9])\.(?:25[0-5]|2[0-4]\d|[1]\d\d|[1-9]\d|[0-9])\.(?:25[0-5]|2[0-4]\d|[1]\d\d|[1-9]\d|[0-9])\.(?:25[0-5]|2[0-4]\d|[1]\d\d|[1-9]\d|[0-9]))(?:\D|$)",
                re.MULTILINE,
            ),
        }
    ]
    ...
    regex.append(
        {
            "types": ["mail", "domain"],
            "regex": re.compile(
                r"((?:[a-zA-Z0-9\/\-\_\.\+]+)@{1}([a-zA-Z0-9\-\_]+\.[a-zA-Z0-9\-\_\.]+)+)",
                re.MULTILINE,
            ),
        }
    )
            ...
            if len(matches) > 0:
                ...
                for found_observable in matches:
                    if isinstance(found_observable, tuple):
                        ...
                        found_observables.append(
                            {"type": r.get("types")[0], "value": found_observable}
                        )
...
def ELKToHiveAlert(TheHiveConnector, alert_data):
    ...
    for found_artifact in found_artifacts:
        ...
        if found_artifact["type"] in defaultObservableDatatype:
            hiveArtifact = TheHiveConnector.craftAlertArtifact(
                dataType=found_artifact["type"],
                data=found_artifact["value"],
                message="",
                tags=found_artifact["tags"],
            )
        else:
            hiveArtifact = TheHiveConnector.craftAlertArtifact(
                dataType="other",
                data=found_artifact["data"],
                message="",
                tags=found_artifact["tags"],
            )
...
def ml2Alert(mlalert):
    ...
    try:
        ...
        logger.info(
            "Looking for ML Alert %s in TheHive alerts", str(mlalert["sourceRef"])
        )
        ...
        if len(results) == 0:
            logger.info(
                "ML Alert %s not found in TheHive alerts, creating it",
            ...
            logger.info(
                "ML Alert %s already imported as alert", str(mlalert["sourceRef"])
            )
...
def logstash2Alert(event):
    ...
    try:
        ...
        # Map the ml watcher alert to the alert that will be enhanced
        logger.info(
            "Looking for Logstash Alert %s in TheHive alerts", str(event["sourceRef"])
        )
        ...
        if len(results) == 0:
            ...
            logger.info(
                "Logstash Alert %s already imported as alert", str(event["sourceRef"])
            )
...
def craftMLAlertDescription(mlalert):
    ...
    description = (
        ...
        + " |\n"
        + "| **Description**         | "
        + str(mlalert["description"].replace("\n", ""))
        + " |\n"
        + "| **ML Influencers**        | "
        + str(mlalert["influencers"].replace("\n", ""))
        + " |\n"
        + "| **ML Type**        | "
        + str(mlalert["type"])
        + " |\n"
