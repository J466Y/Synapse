...
class SplunkConnector:
    ...
    def __init__(self, cfg):
        ...
        self.query_timeout = self.cfg.get(
            "Splunk", "query_timeout", fallback="00:05:00"
        )
    ...
    def initiateConnection(self):
        ...
        try:
            logging.debug(
                "logging into {0} as user {1}".format(
                    self.splunk_base_url, self.username
                )
            )

            client = splunklib.SplunkQueryObject(
                uri=self.splunk_base_url,
    ...
    def query(self, query, start_time=None, end_time=None, relative_time=False):
        ...
        try:
            ...
            if (start_time is not None or end_time is not None) and relative_time:
                search_result = self.client.query_relative(
                    query,
                    relative_duration_before=start_time,
                    relative_duration_after=end_time,
                )
            ...
            if not search_result:
                self.logger.error(
                    "The search has failed please look at the details of the error"
                )
                sys.exit(1)
            else:
