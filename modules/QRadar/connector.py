...
class QRadarConnector:
    ...
    def formatDate(self, qradarTimeStamp):
        ...
        string_formatted_time = formatted_time.strftime("%Y-%m-%d %H:%M:%S")
    ...
    def getOffenses(self, timerange):
        ...
        try:
            if timerange == "all":
                ...
                query = (
                    "siem/offenses?filter=last_updated_time%3E"
                    + str(timeFilter)
                    + "%20and%20last_updated_time%3C"
                    + str(now)
                    + "%20and%20status%3DOPEN"
                )
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    raise ValueError(
                        json.dumps(response_body, indent=4, sort_keys=True)
                    )

            except ValueError:
                self.logger.error(
                    "%s.getOffenses failed, api call returned http %s",
                    __name__,
                ...
                if not self.redis_enabled or self.ip == None:
                    ...
                    try:
                        if response.status_code == 200:
                            ...
                            if self.redis_enabled:
                                self.ip = self.redis_ip.set(
                                    address_id, response_body[field], 604800
                                )
                        else:
                            self.logger.warning(
                                "Couldn't get id %s from path %s (response code %s)"
                                % (address_id, path, response.status_code)
                            )

                    except Exception as e:
                        self.logger.error(
                            "%s.getAddressFromIDs failed", __name__, exc_info=True
                        )
                        raise e
    ...
    def getSourceIPs(self, offense):
        if "source_address_ids" not in offense:
            return []
        addresses_from_ids = self.getAddressesFromIDs(
            "source_addresses", "source_ip", offense["source_address_ids"]
        )
        return addresses_from_ids

    def getLocalDestinationIPs(self, offense):
        if "local_destination_address_ids" not in offense:
            return []
        addresses_from_ids = self.getAddressesFromIDs(
    ...
    def getOffenseTypeStr(self, offenseTypeId):
        ...
        offenseTypeStr = "Unknown offense_type name for id=" + str(offenseTypeId)

        try:
            response = self.client.call_api(
                "siem/offense_types?filter=id%3D" + str(offenseTypeId), "GET"
            )
            response_text = response.text
            response_body = json.loads(response_text)
    ...
    def getOffenseLogs(self, offense):
        ...
        try:
            ...
            last_updated_timeStr = self.formatDate(last_updated_time)
            ...
            query = (
                "select  DATEFORMAT(starttime,'YYYY-MM-dd HH:mm:ss') as Date, UTF8(payload) from events where INOFFENSE('"
                + offense_id
                + "') ORDER BY Date ASC  LIMIT 3 START '"
                + start_timeStr
                + "' STOP '"
                + last_updated_timeStr
                + "';"
            )

            self.logger.debug(
                "Running the enrichment query for the first 3 payloads: {}".format(
                    query
                )
            )
    ...
    def aqlSearch(self, aql_query):
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                self.logger.error(
                    "An error occurred while creating the search: %s, %s"
            ...
            while (response_json["status"] != "COMPLETED") and not error:
                if (
                    ...
                    self.logger.error(
                        "An error occurred while waiting for search completion: %s, %s"
                        % (response.status_code, response.text)
                    )
                    error = True
    ...
    def offenseIsOpen(self, offense_id):
        """
        ...
        try:
            response = self.client.call_api(
                "siem/offenses?filter=id%3D" + str(offense_id), "GET"
            )
            ...
            self.logger.error("QRadar returned http %s", str(response.status_code))
            raise
        except Exception as e:
            self.logger.error(
                "Failed to check offense %s status", offense_id, exc_info=True
            )
            raise Exception(e)
    ...
    def closeOffense(self, offense_id):
        ...
        if self.offenseIsOpen(offense_id):
            try:
                ...
                if response.status_code == 200:
                    self.logger.info("Offense %s successsfully closed", offense_id)
                else:
                    raise ValueError(response_body)
            except ValueError:
                self.logger.error("QRadar returned http %s", str(response.status_code))
                ...
                self.logger.error(
                    "Failed to close offense %s", offense_id, exc_info=True
                )
                raise
        else:
            self.logger.info("Offense %s already closed", offense_id)
