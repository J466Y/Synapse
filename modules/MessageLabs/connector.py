...
class MLabsConnector:
    ...
    def getToken(self):
        ...
        try:
            ...
            self.logger.error(
                "MessageLabs: {}".format(result.get("error")), exc_info=True
            )
            self.logger.error(
                "MessageLabs: {}".format(result.get("error_description")), exc_info=True
            )
            self.logger.error(
                "MessageLabs: {}".format(result.get("correlation_id")), exc_info=True
            )
            raise
    ...
    def scan(self, link_to_load):
        ...
        try:
            ...
            if "@odata.nextLink" in graph_data:
                self.logger.debug(
                    "MessageLabs: Writing nextLink(loop) to the tracker file..."
                )
                next_link = graph_data["@odata.nextLink"]
            elif "@odata.deltaLink" in graph_data:
                self.logger.debug(
                    "MessageLabs: phishing Writing deltaLink(break) to the tracker file..."
                )
            ...
            self.logger.error(
                "Failed to process emails and links", __name__, exc_info=True
            )
            raise
