...
class UsersController(AbstractController):
    ...
    def update(self, user_id, data, fields=None) -> User:
        ...
        patch = AbstractController._clean_changes(
            data, ["name", "organization", "roles"], fields
        )
        return self._wrap(self._api.do_patch(url, patch).json(), User)

    def lock(self, user_id) -> User:
        user = self._api.do_patch(
            "user/{}".format(user_id), {"status": "Locked"}
        ).json()
    ...
    def set_password(self, user_id, password):
        self._api.do_post(
            "user/{}/password/set".format(user_id), {"password": password}
        )
    ...
    def change_password(self, user_id, current_password, new_password):
        self._api.do_post(
            "user/{}/password/change".format(user_id),
