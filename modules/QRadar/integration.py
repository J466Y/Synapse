...
class Integration(Main):
    ...
    def enrichOffense(self, offense):
        ...
        enriched["offense_type_str"] = self.qradarConnector.getOffenseTypeStr(
        ...
        for offense_type, extraction_config in self.cfg.get(
            "QRadar", "observables_in_offense_type", fallback={}
        ).items():
                        ...
                        if len(matches) > 0:
                            ...
                            for match_group, observable_type in extraction[
                                ...
                                try:
                                    artifacts.append(
                                        {
                                            "data": matches[0][match_group],
                                            "dataType": observable_type,
                                            "message": "QRadar Offense Type based observable",
                                            "tags": ["QRadar", "offense_type"],
                                        }
                                    )
                                except Exception:
                                    self.logger.warning(
                                        "Could not find match group {} in {}".format(
                                            match_group, enriched["offense_type_str"]
                                        )
                                    )
                else:
                    self.logger.error(
    ...
    def qradarOffenseToHiveAlert(self, offense):
        ...
        if self.cfg.getboolean("QRadar", "extract_automation_identifiers"):
            ...
            tags_extracted = self.tagExtractor(
                offense,
                self.cfg.get("QRadar", "automation_fields"),
    ...
    def validateRequest(self, request):
        if request.is_json:
            ...
            if "timerange" in content:
                ...
                return (
                    json.dumps(
                        {"sucess": False, "message": "timerange key missing in request"}
                    ),
                    500,
                )
        else:
    ...
    def allOffense2Alert(self, timerange):
        ...
        try:
            ...
            for offense in offensesList:
                ...
                exclusion_regexes = (
                    self.cfg.get("QRadar", "offense_exclusion_regexes", fallback=[])
                    or []
                )
                for offense_exclusion_regex in exclusion_regexes:
                    self.logger.debug(
                        "Offense exclusion regex found '{}'. Matching against offense {}".format(
                            offense_exclusion_regex, offense["id"]
                        )
                    )
                    regex = re.compile(offense_exclusion_regex, flags=re.I)
                    if regex.match(offense["description"]):
                        self.logger.debug(
                            "Found exclusion match for offense {} and regex {}".format(
                                offense["id"], offense_exclusion_regex
                            )
                        )
                        matched = True
                if matched:
                ...
                query = And(
                    ...
                    Eq("source", "Synapse"),
                    Eq("type", "QRadar Offense"),
                )
                self.logger.info(
                    "Looking for offense %s in TheHive alerts", str(offense["id"])
                )
                ...
                if len(results) == 0:
                    ...
                    try:
                        ...
                        self.logger.error(
                            "%s.allOffense2Alert failed", __name__, exc_info=True
                        )
