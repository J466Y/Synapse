...
class Automators(Main):
    ...
    def SendEmailFromAlert(self, action_config, webhook):
        ...
        self.rendered_template = self.renderTemplate(
        ...
        self.data = {
            "data": {"description": self.rendered_template, "title": self.title},
            "dataType": "thehive:case_task",
        }

        self.logger.info("Sending mail for alert with id: %s" % self.alert_id)
        self.CortexConnector.runResponder(
            self.use_case_config["configuration"]["mail"]["responder_id"], self.data
        )
