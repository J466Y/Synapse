        ...
        if not action_performed and 'synapse:create-exception' in tags:
            ...
            if event_id:
                ...
                if original_event:
                    ...
                    rule_name = rules[0] if isinstance(rules, list) and len(rules) > 0 else original_event.get('rule')
                    process_name = original_event.get('process')

                    # 1. Collector Group scoping (Mandatory fallback)
                    if not collector_groups:
                        cg = collector.get('collectorGroup') or original_event.get('collectorGroupName')
                        if not cg:
                            # Try to find collector metadata
                            dev_name = collector.get('device') or original_event.get('device')
                            if dev_name:
                                self.logger.info(f"Fetching collector metadata for {dev_name} to find group.")
                                collectors_result = self.connector.list_collectors()
                                if collectors_result.get('status') and isinstance(collectors_result.get('data'), list):
                        ...
                        if cg:
                            self.logger.info(f"Scoping exception to collector group: {cg}")
                            collector_groups = [cg]
                        else:
                            self.logger.warning(f"Could not determine collector group for event {event_id}.")
                    ...
                    if process_name and rule_name:
                        self.logger.info(f"Scoping exception to process '{process_name}' and rule '{rule_name}'")
                        ...
                        self.logger.warning(f"Could not find process/rule in event {event_id}. Scoping only by group.")
                ...
                comment = f"Exception from TheHive case {case_id.strip('~')} via Synapse"
                ...
                if result.get('status'):
                    action_performed = {'status': True, 'message': f"CreateException: Success for event {event_id}"}
                else:
                    ...
                    if isinstance(data, dict):
                        ...
                        error_msg = str(data)
                    action_performed = {'status': False, 'message': f"CreateException: Failed for event {event_id}. Error: {error_msg}"}
            else:
