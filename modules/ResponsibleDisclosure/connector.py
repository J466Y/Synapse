...
class RDConnector:
    ...
    def __init__(self, cfg):
        ...
        self.cert = self.cfg.getboolean(
            "ResponsibleDisclosure", "verify_cert", fallback=True
        )
    ...
    def getToken(self):
        ...
        try:
            ...
            self.logger.error(
                "ResponsibleDisclosure: {}".format(result.get("error")), exc_info=True
            )
            self.logger.error(
                "ResponsibleDisclosure: {}".format(result.get("error_description")),
                exc_info=True,
            )
            self.logger.error(
                "ResponsibleDisclosure: {}".format(result.get("correlation_id")),
                exc_info=True,
            )
            raise
    ...
    def scan(self, link_to_load):
        ...
        if not link_to_load.startswith("https://graph.microsoft.com"):
            self.logger.error(
                "Blocked potentially malicious SSRF attempt to URL: {}".format(
                    link_to_load
                )
            )
        ...
        try:
            ...
            if "@odata.nextLink" in graph_data:
                self.logger.debug(
                    "ResponsibleDisclosure: will write nextLink(loop) to the tracker file..."
                )
                next_link = graph_data["@odata.nextLink"]
            elif "@odata.deltaLink" in graph_data:
                self.logger.debug(
                    "ResponsibleDisclosure: will write deltaLink(break) to the tracker file..."
                )
    ...
    def sendAutoReply(self, from_email, to_email, template_file_name, subject_name):
        ...
        try:
            ...
            self.send_email_endpoint = self.cfg.get(
                "ResponsibleDisclosure", "send_email_endpoint"
            )
            ...
            self.logger.info(
                "Sent thankyou emails {}".format(graph_data["value"])
            )
    ...
    def moveToFolder(self, email_address, id, folder_name):
        ...
        mailFolders_data = requests.get(
            "https://graph.microsoft.com/v1.0/users/{}/mailFolders".format(
                email_address
            ),
            headers={
                "Authorization": "Bearer " + self.token,
    ...
    def listAttachment(self, email_address, id):
        ...
        try:
            ...
            self.logger.error(e)
            self.logger.error(
                "Failed to retrieve attachment list from email: {}".format(id),
                exc_info=True,
            )

    def downloadAttachments(
        self, attachment_name, attachment_id, isInline, attachment_content_type
    ):
        ...
        try:
            self.attachement_response = requests.get(
                attachment_dwnld_url,
                headers={
                    "Authorization": "Bearer " + self.token,
                    "Content-Type": "application/json",
                },
