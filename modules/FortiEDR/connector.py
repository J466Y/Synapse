...
class FortiEDRConnector:
    ...
    def __init__(self, cfg):
        ...
        try:
            ...
            self.logger.error(
                "Failed to initialize FortiEDR connector: %s", e, exc_info=True
            )
            raise
    ...
    def authenticate(self):
        ...
        try:
            ...
            if not auth_result["status"]:
                self.logger.error(
                    "FortiEDR authentication failed: %s", auth_result["data"]
                )
            ...
            self.logger.error(
                "Unexpected error during FortiEDR authentication: %s", e, exc_info=True
            )
    ...
    def list_events(self, timerange_minutes=None):
        ...
        """
        self.logger.debug(
            "Listing FortiEDR events (timerange: %s min)", timerange_minutes
        )
        try:
    ...
    def list_collectors(self):
        ...
        try:
            ...
            self.logger.error(
                "Failed to list FortiEDR collectors: %s", e, exc_info=True
            )
            return {"status": False, "data": str(e)}
    ...
    def isolate_collector(self, device):
        ...
        try:
            ...
            result = inventory_api.isolate_collectors(
                devices=[device], organization=self.organization
            )
            return result
        except Exception as e:
            self.logger.error(
                "Failed to isolate device %s: %s", device, e, exc_info=True
            )
            return {"status": False, "data": str(e)}
    ...
    def unisolate_collector(self, device):
        ...
        try:
            inventory_api = fortiedr.SystemInventory()
            result = inventory_api.unisolate_collectors(
                devices=[device], organization=self.organization
            )
            return result
        except Exception as e:
            self.logger.error(
                "Failed to unisolate device %s: %s", device, e, exc_info=True
            )
            return {"status": False, "data": str(e)}
    ...
    def remediate_device(self, device, process_id=None):
        ...
        try:
            ...
            result = forensics_api.remediate_device(
                terminatedProcessId=pid, device=device, organization=self.organization
            )
            return result
        except Exception as e:
            self.logger.error(
                "Failed to remediate device %s: %s", device, e, exc_info=True
            )
            return {"status": False, "data": str(e)}
    ...
    def get_event(self, event_id):
        ...
        try:
            ...
            result = events_api.list_events(
                eventIds=[int(event_id)], organization=self.organization
            )
            if (
                result["status"]
    ...
    def create_exception(
        ...
        try:
            ...
            if collector_groups:
                cg = (
                    ",".join(collector_groups)
                    if isinstance(collector_groups, list)
                    else collector_groups
                )
            ...
            if not result.get("status"):
                self.logger.error(
                    "Exception creation FAILED. Full API response: %s",
            ...
            self.logger.error(
                "Failed to create exception for event %s: %s",
                event_id,
                e,
                exc_info=True,
            )
