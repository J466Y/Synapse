...
class Integration(Main):
    ...
    def enrichBreach(self, breach):
            ...
            artifacts.append(
                {
                    "data": hostname,
                    "dataType": "hostname",
                    "message": "Darktrace Device Hostname",
                    "tags": ["Darktrace", "endpoint"],
                }
            )
            ...
            artifacts.append(
                {
                    "data": ip,
                    "dataType": "ip",
                    "message": "Darktrace Device IP",
                    "tags": ["Darktrace", "endpoint"],
                }
            )

        macaddress = breach.get("macaddress") or breach.get("device", {}).get(
            "macaddress"
        )
        if macaddress:
            artifacts.append(
        ...
        for c in components:
            ...
            for f in filters:
                ...
                if "hostname" in filter_type.lower() or "domain" in filter_type.lower():
                    dt_type = "fqdn"
                elif " ip" in filter_type.lower() or filter_type.lower().startswith(
                    "ip "
                ):
                    dt_type = "ip"
                ...
                if dt_type:
                    ...
                    if not any(
                        a["data"] == value and a["dataType"] == dt_type
                        for a in artifacts
                    ):
                        artifacts.append(
                            {
                                "data": value,
                                "dataType": dt_type,
                                "message": f"Darktrace: {filter_type}",
                                "tags": ["Darktrace"],
                            }
                        )
        ...
        tags = ["Darktrace"]
        model_name = (
            breach.get("model", {}).get("name")
            if isinstance(breach.get("model"), dict)
            else breach.get("modelName")
        )
        if model_name:
    ...
    def darktraceBreachToHiveAlert(self, breach):
        ...
        case_template = self.cfg.get(
            "Darktrace", "case_template", fallback="Darktrace Breach"
        )
    ...
    def allBreaches2Alert(self, timerange_minutes=60):
        self.logger.info(
            "Darktrace.allBreaches2Alert starts (timerange: %s min)", timerange_minutes
        )
        ...
        for breach in breaches:
            ...
            query = And(
                Eq("sourceRef", str(pbid)),
                Eq("source", "Synapse"),
                Eq("type", "Darktrace Breach"),
            )
            ...
            if len(results) == 0:
                ...
                try:
                    ...
                    self.logger.error(
                        "Failed to create alert for breach %s: %s",
                        pbid,
                        e,
                        exc_info=True,
                    )
